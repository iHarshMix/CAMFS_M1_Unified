...
2. [Literature Gaps](#2-six-literature-gaps)
3. [Formal Requirements](#3-four-formal-requirements)
...
| **Cross-Boundary Knowledge Benefit** | A rural hospital with only T1 and FLAIR scanners wants its segmentation model to benefit from knowledge patterns learned from T1ce scans at other hospitals — even though it will never acquire T1ce scans itself — because T1ce-derived knowledge about enhancing tumor boundaries can improve predictions made from T1/FLAIR alone. The hospital explicitly consents to absorbing this non-owned modality knowledge. |
...
| Cross-boundary knowledge | Either blocked entirely (hard isolation) or absorbed implicitly/uncontrollably (soft gating) | **Selectively absorbed under explicit consent** — a client may opt in to receive knowledge from non-owned modalities, governed by the same auditable policy matrix |

---

## 2. Six Literature Gaps
...
| **Gap 6 — Consented cross-boundary knowledge absorption** | Every existing method that enforces hard modality isolation (including CAMFS M1's own $\text{Recv}(i) \subseteq O(i)$ constraint) assumes that a client should *never* benefit from a modality it does not own. Soft-gated methods (DisentAFL) allow non-owned modality signal to leak in, but *implicitly and uncontrollably* — there is no consent mechanism, no auditability, and no way for a client to say "I want T1ce knowledge but not T2 knowledge." No existing work provides a **consent-governed, auditable mechanism** for a client to selectively absorb knowledge derived from modalities it does not physically possess. The gap is the binary: either total isolation or uncontrolled leakage — with nothing in between. |

---

## 3. Four Formal Requirements
...
| **R4** | **Cross-Boundary Knowledge Absorption Under Consent** | A client $i$ that does not own modality $m$ ($m \notin O(i)$) but explicitly consents to receive $m$-derived knowledge ($m \in \text{Recv}(i)$) should be able to benefit from model parameters and representations shaped by $m$-data from consenting peers — improving its task performance beyond what its owned modalities alone could achieve — without violating any other client's send constraints, and with the absorption decision recorded in the same auditable policy framework as all other consent decisions. This is **not** equivalent to receiving all non-owned modalities indiscriminately: only those modalities explicitly listed in $\text{Recv}(i)$ are absorbed; all others remain blocked. |
...
| $\text{Recv}(i) \subseteq \mathcal{M}$ | Modalities $i$ wants pooled-in benefit for. **Not bounded by $O(i)$** — may include non-owned modalities the client explicitly consents to absorb knowledge from. Partitions into $\text{Recv}_{\text{own}}(i) = \text{Recv}(i) \cap O(i)$ (owned modalities the client wants federated benefit for) and $\text{Recv}_{\text{cross}}(i) = \text{Recv}(i) \setminus O(i)$ (non-owned modalities whose knowledge the client consents to absorb). |
...
- $\text{Recv}(i) \subseteq \mathcal{M}$: A client may receive benefit for any modality in the universe, **including modalities it does not own**, provided it explicitly consents. *(Relaxed from M1's $\text{Recv}(i) \subseteq O(i)$ — see §R4 and Gap 6.)*
- $\text{Recv}_{\text{own}}(i) = \text{Recv}(i) \cap O(i)$: The subset of received modalities the client also owns (standard federated benefit).
- $\text{Recv}_{\text{cross}}(i) = \text{Recv}(i) \setminus O(i)$: The subset of received modalities the client does **not** own (cross-boundary knowledge absorption). When $\text{Recv}_{\text{cross}}(i) = \emptyset$, this reduces exactly to M1 behavior.
- Degenerate case $\text{Send}(i) = \text{Recv}(i) = O(i)$ recovers the original M1 single-matrix behavior. The relaxed Recv decomposition is a strict generalization of the strict generalization.
